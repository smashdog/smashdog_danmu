import { readTextFile, BaseDirectory, exists, createDir } from '@tauri-apps/api/fs'
import { fetch as httpFetch, Body, ResponseType } from "@tauri-apps/api/http"
let json_file_exists = await exists('gift.json', { dir: BaseDirectory.App })
let gift
if(json_file_exists){
  let giftstr = await readTextFile('gift.json', { dir: BaseDirectory.App })
  gift = JSON.parse(giftstr)
  giftstr = null
}else{
  ...
  let end = res.data.lastIndexOf(')')
  gift = JSON.parse(res.data.slice(start, end)).data
  res = null
  start = null
  end = null
}
...
export default {
  ...
  dgb: function (r) {
    ...
    rt.action = 'gift'
    rt.header = r.ic
    if(gift[r.gfid] &&r.bg && parseInt(r.bg) == 1){
      pc = parseFloat((gift[r.gfid].pc*parseInt(r.gfcnt)/1000).toFixed(2))
    }
    rt.user_title = r.nn
    rt.gift = {
      title: r.gfn,
      ...
      price: pc,
      noble: false,
      big: r.bg && parseInt(r.bg) == 1 && !gift[r.gfid] ? true : false
    }
    return rt
  },
